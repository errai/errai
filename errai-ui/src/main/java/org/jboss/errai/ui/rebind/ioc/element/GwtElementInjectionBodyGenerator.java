...
public class GwtElementInjectionBodyGenerator extends ElementInjectionBodyGenerator {
  ...
  GwtElementInjectionBodyGenerator(final MetaClass type,
  ...
  protected String createInstanceAndConfigure(final List<Statement> statements) {
    ...
    statements.add(declareFinalVariable(elementVar, Element.class,
            Stmt.invokeStatic(Document.class, "get").invoke("createElement", Stmt.loadLiteral(tagName))));

    for (final Property property : properties) {
      statements.add(loadVariable(elementVar).invoke("setPropertyString", loadLiteral(property.name()),
              loadLiteral(property.value())));
    }

    if (!Strings.isNullOrEmpty(spaceSeparatedClassNames)) {
      statements.add(loadVariable(elementVar).invoke("addClassName", loadLiteral(spaceSeparatedClassNames)));
    }
