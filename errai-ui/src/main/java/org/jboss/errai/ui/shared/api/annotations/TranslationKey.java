...
/**
 * <p>
 * An annotation that provides a mechanism for the developer to declare
 * translation strings from within the GWT application code.  This extends
 * the Errai i18n support so that translations can be used from Java code
 * (not just from templates).
 * </p>
 * <p>
 * How does it work?  I'm glad you asked.  The developer must annotate
 * a field which represents the translation key.  This key will then map
 * to a value in the translation bundle file.  Once the field is annotated
 * appropriately, the developer must directly invoke the {@link TranslationService}'s
 * <code>format</code> method.  This method call will perform a lookup in
 * the translation service of the value mapped to the provided key.  Note
 * that value substitution is supported via the {N} format.  See below
 * for some examples.
 * </p>
 * 
...
public @interface TranslationKey {
    ...
    /**
     * Indicates the default value to use if no translation is available for 
     * the current locale.
     */
    String defaultValue();
