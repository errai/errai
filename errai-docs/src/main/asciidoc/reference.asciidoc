...
public class CustomerView {
  ...
  @Inject private TextInput nameText; <1>
  @Inject private NumberInput ageInput; <2>
  // more fields...

  @PostConstruct
  private void init() {
    Customer customer = dataBinder
        .bind(nameTextBox, "name") <1>
        .bind(ageInput, "age") <2>
        .getModel();
  }
}
...
.Native DOM wrappers
====
`TextInput` and `NumberInput` are native DOM wrappers provided by Errai. A DOM wrapper is a `@JsType(isNative=true)` interface for a DOM element. You can learn more about DOM wrappers in the <<sid-51806600,Errai UI>> section.
====

<1> As `TextInput` is a wrapper for an `input[type='text']` element, Errai will correctly infer that it stores a String.
<2> `NumberInput` is a wrapper for an `input[type='number']` element. Errai will also be able to infer that this stores a number.
...
That means that a call to [code]+customer.setName()+ will automatically update the value of the TextBox and any change to the TextBox's value in the browser will update the customer object's name property. So, [code]+customer.getName()+ will always reflect the currently displayed value of the [code]+TextInput+.
...
[source, java]
----
@JsType(isNative=true) <1>
public interface IntegerLabel extends org.jboss.errai.common.client.dom.Label, <2>
                                      org.jboss.errai.common.client.ui.HasValue<Integer> <3>
{
  @JsOverlay <4>
  default Integer getValue() {
    return Integer.valueOf(getTextContent());
  }

  @JsOverlay <4>
  default void setValue(Integer content) {
    setTextContent(content.toString());
  }
}
----

<1> In the JS interop API, native JS types are thin wrappers around JavaScript objects.
<2> The type inherits JS interop mappings for the `HTMLElement` API from the label element interface in Errai.
...
Errai allows you to do this by setting the [code]+onKeyUp+ flag in the [code]+@Bound+ annotation. *_Note that this parameter can only be used on Widgets that extend GWT's ValueBoxBase widget or DOM wrappers backed by input elements_*. Setting this parameter to [code]+true+ will cause the model value to update on a KeyUpEvent as well as the default ValueChangeEvent. This will result in the model object being updated as soon as any text is entered/removed from the Widget. You can specify this as follows:
...
====
Implementing `org.jboss.errai.ui.client.local.api.IsElement` is optional, but we will use it in all of our examples. The interface contains a single default method, `getElement`, that returns the root element of the template, allowing us to programmatically attach a component to the DOM. Later on we will discuss alternative methods of accessing a component's root element.
====
...
<2> This line assumes that `TemplatedBean` implements `org.jboss.errai.ui.client.local.api.IsElement`. The default implementation of `IsElement.getElement` returns the root element of the component as an Errai DOM wrapper, which is compatible with the injected `Document`.
...
<1> `ColorComponent` should be `@Dependent` so that this produces a new instance each time. If a `@Templated` bean has no explicit scope, it is `@Dependent` by default.
<2> Again, we assume `ColorComponent` implements `org.jboss.errai.ui.client.local.api.IsElement`, where `getElement` is defined.
...
The interface [code]+org.jboss.errai.common.client.api.IsElement+ is analogous to `IsWidget`. It declares a single method `getElement` for accessing a type that is a UI component with a root element.

The interface [code]+org.jboss.errai.ui.client.local.api.IsElement+ provides a default implementation of [code]+org.jboss.errai.common.client.api.IsElement+ that works for any templated bean. For example:
...
. JS interop wrapped DOM events on Widgets and Elements
