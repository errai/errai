...
You have two options to set up an Errai application. You can start by copying our errai-tutorial demo or by building an app with the Errai Forge plugin:
...
===== Starting with the Errai Forge plugin

Another way to start a new project with Errai is to use Forge and the Errai Forge plugin. For this approach you will need:

* To download and install link:$$http://forge.jboss.org/[Forge].

* To download the Errai Forge plugin, by either cloning link:$$https://github.com/errai/forge-errai.git[this repo] or downloading and unzipping link:$$https://github.com/errai/forge-errai/archive/master.zip[this file].

===== Loading the Errai Forge plugin

To install the plugin, start forge and then run the command, where `$PATH_TO_PLUGIN` is the absolute path of the Errai Forge plugin source you have previously downloaded.

----
forge source-plugin $PATH_TO_PLUGIN
----

===== Creating your new project

While still in Forge, run this command:

----
new-project --named $PROJECT_NAME --topLevelPackage $PROJECT_PACKAGE --projectFolder $PROJECT_PATH
----

Make sure to replace the above variables so that:

* *$PROJECT_NAME* is the name of your new project.

* *$PROJECT_PACKAGE* is the Java package used by your new project.

* *$PROJECT_PATH* is the path to the root directory of your new project (Forge will create the directory for you).

===== Installing the Errai Forge plugin

In Forge (while in your new project's root directory) run
...
* The logical name of the link:$$http://www.gwtproject.org/doc/latest/DevGuideOrganizingProjects.html#DevGuideModuleXml[GWT module] for your project. (So if you want a module called _App_ in the package `org.jboss.errai`, the logical name would be `org.jboss.errai.App`)
...
This will configure the main dependencies and plugins required to run any Errai App in GWT Development Mode and to compile for deployment. You will now also have access to all of the Errai Forge plugin commands. Since one of the proceeding sections will be describing the use of Errai CDI, you should install this feature now by running

----
errai add-feature cdi
----
...
6. Next you will need to setup a Maven Run Profile for Development Mode. To do so select _Run As..._ > _Run Configurations..._ from the toolbar.

.Run Configurations
image::author/download/attachments/54493220/RunConfigurations.PNG[]

7. Select _Maven Build_ from the sidebar and create a new launch configuration by pressing the _New_ button in the top left corner.

.New Configuration
image::author/download/attachments/54493220/NewLaunchConfiguration.PNG[]

8. Give the configuration a name, then click _Browse Workspace_ and select the root directory of your new project.

.Select Project Root Directory
image::author/download/attachments/54493220/LaunchRootDir.PNG[]

9. In the _Goals_ text box, type "clean gwt:run". Click _Apply_ to save the configuration and then _Close_.

.Run Configurations Goals
image::author/download/attachments/54493220/GwtRunGoals.PNG[]

10. You can add this new configuration under the _Run As_ button in your toolbar by selecting _Run As_ > _Organize Favorites_, then clicking _Add_ and selecting the run configuration.

.Add Configuration to Favourites
image::author/download/attachments/54493220/OrganizeFavourites.PNG[]

11. At this point, you should try running your new configuration to make sure everything is in working order. To run your app, find the run configuration under the _Run As_ menu in the toolbar.

.Run Gwt Development Mode from Eclipse
image::author/download/attachments/54493220/UseGwtRunConfiguration.PNG[]
...
12. To setup a debug run configuration for GWT Development Mode, repeate steps (7) and (8) above, but this time use the _Goals_ "clean gwt:debug".

.Configure Maven Debug Configuration
image::author/download/attachments/54493220/ConfigureGwtDebug.PNG[]

13. Next we will need to setup our remote debugger configurations in Eclipse. Because the client and server code run on separate JVMs, we will need to setup two such configurations. To create a debug configuration, select _Debug As..._ > _Debug Configurations..._ from the toolbar.

.Create Debug Configuration
image::author/download/attachments/54493220/DebugConfigurations.PNG[]

14. In the sidebar, select _Remote Java Application_ and click the _New_ button in the top right corner.

.Create Remote Debug Configuration
image::author/download/attachments/54493220/NewDebugConfig.PNG[]

15. This new configuration will be for remote debugging your client-side code, so give it an appropriate name. If the name of your project is not already in the _Project_ field, click _Browse_ and select it. The _Host_ and _Port_ values should be _localhost_ and _8000_ respectively, such that your configuration looks like this:

.Client Debug Configuration
image::author/download/attachments/54493220/ClientDebugConfig.PNG[]

If everything is correct, click _Apply_.

16. Create another _Remote Java Application_ run configuration with the steps just described for remote debugging server code. The only differences from the client configuration should be the name and the port, which is _8001_. Thus the server remote debug configuration should look like this:

.Server Debug Configuration
image::author/download/attachments/54493220/ServerDebugConfig.PNG[]

17. That's it! You've successfully imported your Errai project into Eclipse. Now, on to <<sid-54493502,coding>> !
