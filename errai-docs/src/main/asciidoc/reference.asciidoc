...
Unfortunately, Errai Navigation's writes page parameter values _after_ Errai Data Sync reads them, so Data Sync cannot
refer to page parameter fields directly. Your application is responsible for setting fields before the `@PostConstruct`
method returns. We hope to remedy this issue in a future release (community participation welcome!) 
====
