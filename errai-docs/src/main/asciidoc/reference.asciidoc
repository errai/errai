...
Another way to start a new project with Errai is to use Forge and the Errai Forge Addon. To use this method, follow the instructions link:$$https://github.com/errai/forge-errai/blob/master/README.asciidoc$$[here] to install the Errai Forge Addon and create a new project.
...
====
It is important to note that the [code]+ClientExceptionMapper+ will only be invoked when the callback passed to the Caller is an instance of RestErrorCallback.
====
