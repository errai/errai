...
. Setup a Keycloak server. Please consult the link:$$http://keycloak.jboss.org/docs.html$$[Keycloak documentation] for details on how this is achieved.
  ...
  .. After creating your application select it from the list and make sure to adjust the following settings as appropriate:
    ...
    * **Scope** -- Move any roles that users in your app *may* possess to the "Assigne Roles". (Note: moving the roles to "Assigned Roles" does not mean *all* users will have this role. It does mean that these roles will be visible to your app for users that do have them.)
...
. Configure the `ErraiUserExtractionFilter` in your `web.xml`. All that is necessary is adding a filter-mapping for your GWT host page like so:
+
[source,xml]
----
  <filter-mapping>
    <filter-name>ErraiUserExtractionFilter</filter-name>
