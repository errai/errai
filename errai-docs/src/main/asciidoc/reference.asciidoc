...
.Native DOM wrappers
====
`TextInput` and `NumberInput` are native DOM wrappers provided by Errai. A DOM wrapper is a `@JsType(isNative=true)` interface for a DOM element. You can learn more about DOM wrappers in the <<Errai UI>> section.
====
...
====
The above rules for inferring values are those used when you directly use the data-binding API. Using the declarative API it is possible create custom JS interop interfaces for DOM elements where you can specify how values are bound to the element (see <<Customizing Element Binding Behaviour>>).
====
