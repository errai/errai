...
There are two kinds of UI components you can build with Errai UI: composite components and non-composite components. Composite components extends the [code]+com.google.gwt.user.client.ui.Composite+ class, making the components widgets. Non-composite components do not require extending any base class and can be used without widgets.
...
====
Since Errai 4.0, there is no longer a required base class for Errai UI components. In fact, extending [code]+Composite+ is deprecated.
====

Composite components behave no differently from any other GWT Widget once built. The primary difference is in A) their construction, and B) their instantiation. As with most other features of Errai, dependency injection with CDI is the programming model of choice, so when interacting with components defined using Errai UI, you should always [code]+@Inject+ references to your Composite components.

[[sid-51806600_ErraiUI-Injectasingleinstance]]

==== Inject a single composite component instance

In this example we inject a single composite component and add it to the [code]+RootPanel+.
...
==== Inject a single non-composite component instance

In this example we inject a single non-composite component and add it to the DOM.
...
public class Application {
   @Inject
   private NonComposite comp;

   @PostConstruct
   public void init() {
     Document.get().getBody().appendChild(comp.getRootElement());
   }
}
----

[NOTE]
.Manually attaching non-composite components
====
In order to manually attach a non-composite component to the DOM it is necessary access the root element of the template. How this is done will be demonstrated later on when we discuss using [code]+@DataFields+.
====
...
Here we create and add many composite components to the [code]+RootPanel+ using an injected [code]+Instance+.
...
Here is a basic composite component.
...
Here is the same component again as a non-composite.
...
We will begin by creating a simple HTML login form to accompany our [code]+@Templated LoginForm+ component.
...
=== Use other Widgets in a component

Now that we have created the @Templated component and an HTML template, we can start wiring in functionality and behavior; this is done by annotating fields and methods to replace specific sub-elements of the template DOM with other Widgets. We can even replace portions of the template with other Errai UI components!

[[sid-51806600_ErraiUI-AnnotateWidgetsinthetemplatewith@DataField]]

==== Annotate Widgets or Elements in the template with @DataField

In order to composite widgets or elements into the template DOM, you annotate fields in your @Templated Composite component with @DataField, and mark the HTML template Element with a correspondingly named [code]+data-field+, [code]+id+, or [code]+class+ attribute. This informs Errai UI which element in the template the widget or element should replace. All replacements happen while the @Templated Composite component is being constructed; thus, fields annotated with @DataField must either be [code]++@Inject++ed or provide their own Widget or Element instances in field initializers.
...
The previous [code]+LoginForm+ example uses [code]+Widgets+ and a DOM element from gwt-user, but there are other types that can be used as [code]+@DataFields+. Here is a full list:

. Any [code]+com.google.gwt.user.client.ui.Widget+ type.

. Any [code]+com.google.gwt.dom.client.Element+ type.

. Any Errai UI component (composite or non-composite).

. Any element interfaces from gwt-elemental.
...
==== Accessing the root element of a non-composite template

Since non-composite templated beans are not widgets, it is necessary to access their HTMLElement roots when programmatically adding them to the DOM. Errai UI provides two ways of doing this.
...
public class Setup {
  ...
  public void attachBean() {
    doc.getBody().appendChild(bean.getElement());
  }
}
----
...
====
The `getElement` call in the usage above returns the element as the type [code]+org.jboss.errai.common.client.dom.HTMLElement+.
====
...
.Jsinterop ahead
====
This section assumes basic familiarity with jsinterop in GWT 2.8+.
====

Since 4.0, Errai UI supports working with jsinterop wrappers of DOM elements. Additionally, Errai provides a concise way of allowing your custom jsinterop wrappers to be injected into components.

===== LoginForm with a jsinterop element

Let's redo our previous login form, but with a new jsinterop form element as the root. First we need our element wrapper.
...
Here we have a defined a jsinterop interface that will be backed by a FormElement javascript object at runtime. The [code]+org.jboss.errai.ui.shared.api.annotation.Element+ annotation tells the Errai DI container to call `document.createElement("form")` in javascript to satisfy injection sites for this type.
...
----
/*
* Like the component from the previous example, but with a native element as its root.
*/
@Templated("#form")
public class LoginForm {
   // Now are root element is a native JsType
   @Inject
   @DataField
...
In our previous examples injecting [code]+Elements+ or native [code]+JsTypes+, we did so with types that had a single possible tag name. But consider the heading tags (h1, h2, ..., h6). In gwt-user these are all represented by [code]+HeadingElement+. We might also wish to make our own [code]+JsType+ wrapper that also represents these tags:
...
For both [code]+Elements+ and native [code]+JsTypes+ one can use the [code]+javax.inject.Named+ annotation to specify a tag name at a injection site.
...
. Template element inner text and inner HTML are preserved when the given [code]+@DataField+ is a [code]+Widget+ that implements [code]+HasText+ or [code]+HasHTML+ or if it is a DOM element that has no children.
...
====
As with previous features, the [code]+DivElement+ above could be replaced with a native [code]+JsType+.
====
