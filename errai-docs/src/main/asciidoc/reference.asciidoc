...
====
The above methods are implemented with Errai RPC calls to the service `org.jboss.errai.security.shared.AuthenticationService`. However, client-side interceptors are used for caching so that generally only calls to `login` and `logout` must be sent over the wire.
====
...
The annotations `@RequireAuthentication` and `@RequireRoles` are the only two annotations necessary to secure a resource or UI element. In general, `@RequireAuthentication` blocks a resource from users who are not logged in and `@RequireRoles` blocks a resource from users who do not posess all the roles (a parameter of the annotation, given as an array of Strings). Below we will explain how different resources are blocked from unauthorized users.
...
Any class annotated with `@Page` can also be marked with `@RequireAuthentication` and `@RequireRoles`. By doing so, users will be prevented from navigating to the given page if they are not logged in or lack authorization.
...
Security checks performed before page navigation do not use any RPC calls, but are instead performed from a cached (in-memory) instance of the `org.jboss.errai.security.shared.User`. This prevents the possibility of lengthy delays between page navigation while waiting for RPC return values.
...
One option offered by Errai is to persist the `org.jboss.errai.security.shared.User` object in the browsers local storage. This can be done by adding the following to `ErraiApp.properties`:
...
Errai Security annotations can also be used to hide Errai UI template fields. When a user is not logged in or lacks required roles the annotated field will have `display: none` added to it's CSS style.
...
All Errai Security authentication is implemented with Errai Remote Procedure Calls to the `AuthenticationService`. The default implementation of this interface uses PicketLink, but it is possible to create and use an alternative implementation by following these steps:

. Create an implementing class for `AuthenticationService`.

. Annotate your class with `@Alternative`.

. Add the following to your `beans.xml` (replacing the fully-qualified class name):
+
[source,xml]
----
<alternatives>
  <class>org.jboss.errai.example.AuthenticationServiceImpl</class>
</alternatives>
----
...
Errai security requires a CDI container to intercept calls to remote services. In particular, the following interceptors must be added to your applications `beans.xml`:
