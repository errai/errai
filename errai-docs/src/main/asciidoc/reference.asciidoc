...
==== Debugging the app in GWT's development mode
...
===== Debugging in your IDE using the EmbeddedWildFlyLauncher
...
====== Configuring Eclipse to use the embedded WildFly launcher
...
4. Select _Super Development Mode_ in the GWT tab, and add your web app's module under _Available Modules_.
+
.Dev Mode Configuration: GWT tab
image::author/download/attachments/gpe/EclipseWebAppGWT.png[width=650]

5. Under the _Arguments_ tab, enter the following arguments. Replace the war directory and the fully qualified app name with those of your own application:
+
.Program Arguments
----
-superDevMode -remoteUI "${gwt_remote_ui_server_port}:${unique_id}" -logLevel INFO -codeServerPort 9997 -port 8888 org.jboss.errai.demo.App -war /home/ddadlani/errai-tutorial/src/main/webapp -server org.jboss.errai.cdi.server.gwt.EmbeddedWildFlyLauncher org.jboss.errai.demo.App
----
+
.VM arguments
----
...
----
+
where `errai.jboss.home` points to your own WildFly installation.
+
...
====
If you wish to download and install WildFly into the _target/_ directory of your web app, you will need to run `mvn process-resources` prior to running or debugging your app. To do this, you can set up a Maven build configuration in Eclipse and specify _Goals_: "process-resources".
====
...
To debug your client-side JavaScript from within Eclipse using the embedded WildFly launcher, you will need to use the SDBG plugin for GWT Super Dev Mode. More information can be found http://sdbg.github.io/[here].

===== Configuring Intellij IDEA (Ultimate Edition) to use the embedded WildFly launcher
...
----
+
where `errai.jboss.home` points to your WildFly installation directory.
+
.Dev Mode Parameters
----
...
.Copy of WildFly in the target/ directory
====
If you wish to download and install WildFly into the _target/_ directory of your web app, you will need to run `mvn process-resources` prior to running or debugging your app. To do this, you can add a Maven goal to the _Before Launch_ task list. Click the `+` button as before, select _Run Maven Goal_ and in the _Command Line_ field, enter "process-resources". Ensure that the working directory is the same as your project directory before clicking "OK".
====
