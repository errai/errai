...
public class JaxrsProxyGenerator {
  ...
  private void generateProxyMethod(ClassStructureBuilder<?> classBuilder, MetaMethod method) {
    ...
    String path = "Path:" + rootResourcePath + ((subResourcePath != null) ? subResourcePath.value() : "");
    classBuilder
        .publicMethod(method.getReturnType(), method.getName(),
            DefParameters.from(method).getParameters().toArray(new Parameter[0]))
        .append(Stmt.loadStatic(System.class, "out").invoke("println", path))
