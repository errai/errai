...
public class UserHostPageFilter implements Filter {
  ...
  String securityContextJson(final User user) {
    final String userJson = escapedJson(user);

    return "{\"" + SecurityConstants.DICTIONARY_USER + "\": \"" + userJson + "\"}";
  }

  String escapedJson(final User user) {
    final String rawJson = ServerMarshalling.toJSON(user);

    return rawJson.replaceAll("\\\\\"","\\\\\\\\\"").replaceAll("\"","\\\\\"");
  }
